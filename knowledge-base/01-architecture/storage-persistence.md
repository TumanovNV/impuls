...
1.4.16 (IMP-39) добавляет необязательный ключ `file` для закреплённого локального файла: читаемый путь остаётся в `text`, а `file.bookmark` — опциональные bookmark-байты. Ключ пишется только у file pin, поэтому файл из одних текстовых элементов кодируется байт-в-байт как раньше, а старый `snippets.json` декодируется без изменений — это additive optional field, а не миграция. Содержимое самого файла не сохраняется никогда; хранится только ссылка.

**Privacy class bookmark'а выше, чем у пути, и это важно.** Bookmark ~1.1 KiB и содержит в открытом виде не только сам путь, но и имя тома (`Macintosh HD`), **UUID тома**, inode и продублированный путь в нижнем регистре. Поэтому:
