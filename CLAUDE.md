...
- **Path-scoped rules load themselves.** `.claude/rules/*.md` carry `paths:` frontmatter, so `swift-ui.md`, `localization.md`, `release.md`, `qa-impact.md`, `website.md` and `legal-privacy.md` arrive only when you touch the matching files. Do not read them pre-emptively, and do not copy their content anywhere else. Website, legal and app localization are three separate contracts — `knowledge-base/04-development/localization.md` is the canonical route before changing the supported-language set.
