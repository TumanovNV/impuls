...
`FileToolsCoordinator` запускает свои batch-операции через `Task.detached` и не сохраняет handle, поэтому отменить их при закрытии окна или teardown нельзя — длинная пачка конвертаций доработает до конца. Внутри пачки нет `Task.checkCancellation()`. Практический риск ограничен: операции пользователь запускает явно, каждая обрабатывает один файл за итерацию в `autoreleasepool`, partial failures аккумулируются и показываются. Зафиксировано аудитом 1.4.12 как известный пробел, а не исправлено: добавление отмены — изменение контракта модуля, которому нужен собственный design review и тесты на частично выполненную пачку.
