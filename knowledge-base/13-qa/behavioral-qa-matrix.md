...
| MUS-09 | Web player content process terminates | mixed | when WebKit's content process dies — a crash or the system reclaiming it under memory pressure — the panel clears the last track instead of leaving it on screen with transport buttons that do nothing; the failure is a fixed, page-free message; **no automatic reload** is attempted, so a repeatedly crashing page cannot become a retry storm; the next explicit Open or Reload rebuilds the player normally. Handler behaviour is deterministic-tested; what needs a real Mac is only that WebKit actually delivers the callback |
