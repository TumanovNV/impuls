...
| New user-visible platform/hardware lifecycle edge | Behavioral QA Matrix | automated test where possible + manual/mixed QA row with expected contract |
...
| New permission | permission architecture + TCC + QA matrix | UI explanation, entitlement, privacy/security, audit, PERM rows, CI |
...
| Update/release | update system + ADR-005 + QA matrix | workflows, bundle/dmg, Sparkle flags, REL rows, artifacts, security audit |
...
6. Изменился ли ownership важного type — нужно ли регенерировать map?
7. Затронут ли private production runtime, который принадлежит `office-it-docs`, а не public repo?
