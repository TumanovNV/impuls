...
| PWR-15 | Adapter Current | automated + manual-hardware | Deterministic tests establish that optional positive integer mA comes only from public `IOPSCopyExternalPowerAdapterDetails` / `kIOPSPowerAdapterCurrentKey`; missing, zero, negative, malformed, no-adapter and desktop cases stay absent. It adds neither polling nor a new source, and is never reinterpreted as charging power: rated adapter watts and battery-side derived W stay separate. Manual hardware QA may verify display only when the specific adapter/macOS publishes the key; absence alone is not failure. |
| PWR-16 | Stay Awake / Keep Display Awake | mixed | Deterministic tests establish the lease model: one physical `PreventUserIdleSystemSleep` assertion however many leases ask for it, an independent `PreventUserIdleDisplaySleep` assertion, release only of the releasing owner's own token, all-or-nothing reconciliation so a failed display assertion never costs a working system one and a partial acquisition leaves no orphan, a monotonic deadline whose superseded expiry cannot end a newer mode, a duration change that neither duplicates the assertion nor keeps the old deadline, idempotent shutdown, and a fresh instance that is off because nothing is persisted. Unit tests run entirely against a fake assertion backend and never create a real one. Manual hardware QA verifies the rest on a real Mac with read-only `pmset -g assertions`: the named Impuls assertion is absent while off, present while on, gone after Turn Off and after Quit; the display assertion appears only on the explicit option; the mode is off after relaunch; the battery caution appears on battery and not on AC; and Impuls starts no `caffeinate` child process and writes no `pmset`/Energy Saver setting. |
...
| DATA-14 | Local file pinned in Snippets (IMP-39) | mixed | Automated: reference round-trip and pre-IMP-39 decode, a rename followed through the bookmark, the write-back updating in place without duplicating an identity, missing file resolves unavailable and Copy/Open/Reveal fail closed, re-select edits the same row without duplicating, directories/packages/dead symlinks rejected and live symlinks accepted, a backup carrying the path but never the bookmark, and removing a pin leaving the file on disk. Needs a real Mac — none of this is covered by a test: single click copies the file so ⌘V in Finder pastes the *file*, double click opens it once with no copy, the hover ✕ removes the pin rather than copying it, right-click opens the context menu on a file row, the first click after focus was elsewhere is not swallowed, a press-drag-release does not copy, drag-and-drop onto both a populated and an empty list, Reveal in Finder, an external volume unmount/remount, and — because the Snippets list became the app's only `LazyVStack` — Tab traversal and VoiceOver enumeration through a list long enough to scroll. Product owner records the manual result. |
...
| MUS-10 | Spotify native scripting on a real Mac | manual-macos | With Spotify installed and selected: no automatic TCC prompt; explicit Allow enables metadata, sane duration/position, play/pause, next, `previous track` and seek. Verify closed/reopened, denied/revoked Automation, Apple Music isolation, fold/open stops fast refresh, and no Spotify WebKit or Impuls network request. Product owner records the manual result after review. |
...
| SUP-02 | Settings voluntary-support surface | manual-macos | Settings → Feedback → Support the Project shows voluntary wording, states that the amount is chosen on an external service and that Impuls features do not depend on support, offers separate `Russia — CloudTips` and `Other Countries — Boosty` actions, and says the page opens in the default browser. No urgency, guilt language or promised advantage appears. |
| SUP-03 | Explicit CloudTips support action | manual-macos | Clicking CloudTips explicitly opens exactly `https://pay.cloudtips.ru/p/e04ac53f` in the system default browser. Impuls does not choose it automatically, open an embedded payment surface or make an availability request. Do not make a test payment or capture account/payment details. |
| SUP-04 | Explicit Boosty support action | manual-macos | Clicking Boosty explicitly opens exactly `https://boosty.to/tumanovnv/donate` in the system default browser. Impuls does not choose it automatically, open an embedded payment surface or make an availability request. Do not make a test payment or capture account/payment details. |
| SUP-05 | Return from voluntary-support provider | manual-macos | Returning to Impuls changes no feature, automatic-prompt state, meaningful-use counter, provider selection or entitlement. Impuls receives no amount, payment result or provider account state. |
| SUP-06 | No embedded payment UI | manual-macos | Both voluntary-support actions leave Impuls and use the default browser. No payment WebView, SDK UI, card field, SBP credential field, callback or background payment activity appears in the app. |
| SUP-07 | Approved provider endpoints before release | manual-macos | Before release, a human verifies that both exact approved URLs still resolve to the intended CloudTips page and the intended Boosty profile. Verification stops before payment and records no screenshot or evidence containing account, payment or private details. |
