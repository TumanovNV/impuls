...
@MainActor
final class IORegistryAccessoryProviderTests: XCTestCase {
    func testAnEmptyMacIsAReadyProviderWithNoDevicesAndNoError() async {
        let source = FakeAccessorySource(devices: [])
        let provider = IORegistryAccessoryProvider(source: source)
    ...
    private func firstUpdate(
        from provider: IORegistryAccessoryProvider,
